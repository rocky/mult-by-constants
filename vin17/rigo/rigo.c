...
The steps are the following:
    ...
    Minor modifications by Vincent Lefèvre, in 2004, 2005
    (SPACES project, Loria / INRIA Lorraine), and in 2013
    (AriC project, INRIA Grenoble Rhône-Alpes / LIP, ENS-Lyon).
...
/*
    Not used in the program but it can be useful.
*/
...
int Compute_Weights_Unique(char *number, weight_r *result, const long length)
{
  ...
  //so we can store the bounds on the distances in a "relative" array
  //negative indices correspond to N-P / P-N
  //positive indices correspond to P-P / N-N
  ...
  for (i = - length + 1; i < length; i++)
    {
      ...
      //for which the bound is < to the current best weight
...
{
  //distance between the 2 occurrences of the pattern
  int distance, i, temp;
...
/*
  ...
  return values:
  - number1: contains the remainder when the pattern has been subtracted
  - number2: same
  return val: nothing.
*/
      ...
      if (build_data[i].str1 == build_data[i].str2)
        {
          ...
          //composed by the 2 occurrences of the pattern
